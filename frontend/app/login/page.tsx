...
export default function LoginPage() {
  ...
  // Separated into two effects to avoid timer cleanup issues
  useEffect(() => {
    ...
    if (!loading && user) {
      ...
      console.log('[LoginPage] User authenticated via context, scheduling redirect to dashboard');
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (event, session) => {
        ...
        if (event === 'SIGNED_IN' && session) {
          // User just signed in, let the context effect handle redirect
